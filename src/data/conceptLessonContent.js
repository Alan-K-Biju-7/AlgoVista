...
const lessonCopy = {
  array: {
    ...
    complexity: { time: 'Direct access O(1); a full scan is O(n); pattern-specific work varies.', space: 'O(1) extra for in-place scans, or O(n) when building auxiliary state.' },
  ...
  complexity: {
    ...
    complexity: { time: 'Analyze the dominant operation as input size n grows.', space: 'Count auxiliary structures and the maximum recursion depth.' },
  ...
  search: {
    ...
    complexity: { time: 'O(n) for a linear scan; O(log n) when ordered or monotonic structure safely halves the space.', space: 'Usually O(1) iteratively; recursive forms may use call-stack space.' },
  ...
  matrix: {
    ...
    complexity: { time: 'A full traversal is O(rows × columns); structured searches can do less.', space: 'Often O(1) extra unless visited state or output storage is required.' },
  ...
  string: {
    ...
    complexity: { time: 'A direct scan is O(n); matching and repeated substring work depend on the chosen pattern.', space: 'O(1) for fixed alphabets or O(k) for tracked characters and prefixes.' },
  ...
  recursion: {
    ...
    complexity: { time: 'Estimate branching factor raised to recursion depth, then account for pruning.', space: 'O(depth) call stack plus the current path or decision state.' },
  ...
  'linked-list': {
    ...
    complexity: { time: 'Traversal and search O(n); insertion at a known node O(1).', space: 'O(1) extra for iterative pointer operations; O(n) for stored nodes.' },
  ...
  stack: {
    ...
    complexity: { time: 'Push, pop, and peek are O(1); processing n inputs is usually O(n).', space: 'O(n) in the worst case for stored unfinished work.' },
  ...
  queue: {
    ...
    complexity: { time: 'Enqueue and dequeue are O(1) with a proper queue or deque.', space: 'O(n) in the worst case for queued work.' },
  ...
  hashing: {
    ...
    complexity: { time: 'Insert and lookup average O(1), worst-case O(n) under heavy collision.', space: 'O(n) for keys, counts, or cached state.' },
  ...
  sorting: {
    ...
    complexity: { time: 'Comparison sorts range from O(n²) elementary methods to O(n log n) efficient methods.', space: 'Varies from O(1) in-place to O(n) auxiliary storage; recursion may add O(log n).' },
  ...
  tree: {
    ...
    complexity: { time: 'A traversal is O(n); balanced search/update O(log n), but a skewed tree can be O(n).', space: 'O(h) traversal stack where h is tree height, plus stored nodes.' },
  ...
  graph: {
    ...
    complexity: { time: 'BFS and DFS are O(V + E); shortest-path and spanning-tree algorithms vary.', space: 'O(V + E) for an adjacency list plus O(V) frontier and visited state.' },
  ...
  greedy: {
    ...
    complexity: { time: 'Often O(n log n) when sorting choices, then O(n) to select; problem-specific.', space: 'Usually O(1) to O(n), depending on ordering and frontier structures.' },
  ...
  dp: {
    ...
    complexity: { time: 'Number of reachable states × transition work per state.', space: 'Number of stored states; rolling-state optimization may reduce dimensions.' },
  ...
  bits: {
    ...
    complexity: { time: 'O(1) for a fixed-width machine word, or O(b) when scanning b bits.', space: 'Typically O(1) extra unless generating many masks or subsets.' },
  ...
  divide: {
    ...
    complexity: { time: 'Use a recurrence: number of subproblems × smaller-work cost + combine cost.', space: 'Recursion depth plus temporary storage used by the combine step.' },
  ...
  foundation: {
    ...
    complexity: { time: 'Derive from how many inputs, states, or operations the implementation visits.', space: 'Count only extra memory beyond the input, including recursive calls.' },
