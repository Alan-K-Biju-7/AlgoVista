...
function ProgressBanner({ allProblems, getStatus }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:'1rem', padding:'0.75rem 1rem', borderRadius:'0.6rem', background:'var(--bg-card)', border:'1px solid var(--border-default)', marginBottom:'1.5rem' }}>
      ...
      <div style={{ flex:1, height:'6px', borderRadius:'999px', background:'var(--border-default)', overflow:'hidden' }}>
        <div style={{ height:'100%', width: pct + '%', background:'#00d4aa', borderRadius:'999px', transition:'width 0.4s' }} />
      </div>
      <span style={{ fontSize:'0.82rem', fontWeight:'700', color:'#00d4aa' }}>{solved}/{all.length}</span>
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', maxWidth: '1200px', margin: '0 auto', padding: '2rem 1.5rem', gap: '1.75rem' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{marginTop:'0.75rem'}}><ProgressBanner allProblems={ALL_PROBLEMS} getStatus={getStatus} /></div>
        </div>
        {activeProblem ? (
          <ProblemDetail
