  ...
  const btn = (label, onClick, disabled = false, variant = '') => (
    <button key={label} onClick={onClick} disabled={disabled}
      style={{
        ...
        border: variant === 'danger' ? '1px solid #7f1d1d' : '1px solid var(--border-default)',
        background: variant === 'primary' ? 'var(--accent-dim)' : variant === 'danger' ? '#1c0a0a' : 'var(--bg-elevated)',
        color: variant === 'primary' ? '#fff' : variant === 'danger' ? '#fca5a5' : 'var(--text-secondary)',
    ...
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)', padding: '1.1rem' }}>
      ...
      <div style={{ marginBottom: '0.9rem' }}>
        ...
        <input type="number" value={inputVal} onChange={(e) => setInputVal(e.target.value)}
          onKeyDown={handleKey} placeholder="e.g. 42" />
      </div>
