export default function BellmanFordControls({
  ...
  onRun, onStepForward, onStepBack, onStop, onReset, onLoadPreset,
  speed, setSpeed, isRunning, stepIndex, totalSteps, message,
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-xl)', padding: '1.1rem' }}>
      ...
      <p style={sLbl}>Preset graphs</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
        {[
          ['default',  'Default (CLRS classic)'],
          ['simple',   'Simple 4-node'],
          ['negCycle', '⚠ Negative cycle'],
        ].map(([key, label]) => (
          <button key={key} onClick={() => onLoadPreset(key)} disabled={isRunning} style={{
            padding: '0.32rem 0.6rem', borderRadius: '0.4rem', fontSize: '0.75rem', fontWeight: '500',
            border: key === 'negCycle' ? '1px solid rgba(239,68,68,0.35)' : '1px solid var(--border-strong)',
            background: key === 'negCycle' ? 'rgba(239,68,68,0.05)' : 'var(--bg-elevated)',
            color: key === 'negCycle' ? '#fca5a5' : 'var(--text-secondary)',
      ...
      <div style={{ display: 'flex', gap: '0.35rem' }}>
        {[['Slow', 900], ['Normal', 500], ['Fast', 180]].map(([l, v]) => (
      ...
      <p style={sLbl}>Add directed edge</p>
      <div style={{ display: 'flex', gap: '0.3rem', flexWrap: 'wrap', alignItems: 'center' }}>
        <input value={addEdgeFrom} onChange={(e) => setAddEdgeFrom(e.target.value.toUpperCase())} placeholder="A" maxLength={3} style={{ width: '38px', padding: '0.38rem 0.4rem', fontSize: '0.8rem', background: 'var(--bg-elevated)', border: '1px solid var(--border-strong)', borderRadius: '0.35rem', color: 'var(--text-primary)' }} />
        <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>→</span>
        <input value={addEdgeTo} onChange={(e) => setAddEdgeTo(e.target.value.toUpperCase())} placeholder="B" maxLength={3} style={{ width: '38px', padding: '0.38rem 0.4rem', fontSize: '0.8rem', background: 'var(--bg-elevated)', border: '1px solid var(--border-strong)', borderRadius: '0.35rem', color: 'var(--text-primary)' }} />
        <input type="number" value={addEdgeWeight} onChange={(e) => setAddEdgeWeight(e.target.value)} placeholder="w" style={{ width: '44px', padding: '0.38rem 0.4rem', fontSize: '0.8rem', background: 'var(--bg-elevated)', border: '1px solid var(--border-strong)', borderRadius: '0.35rem', color: 'var(--text-primary)' }} />
      ...
      <div style={{ marginTop: '0.8rem', padding: '0.6rem 0.75rem', borderRadius: '0.45rem', background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', minHeight: '3.2rem' }}>
        <p style={{ fontSize: '0.76rem', color: '#a5b4fc', lineHeight: 1.65 }}>{message}</p>
      </div>

      <div style={{ marginTop: '0.6rem' }}>{btn('Reset graph', onReset)}</div>
    </div>
  );
}
