export default function HashControls({
  keyInput, setKeyInput, valInput, setValInput,
  onInsert, onSearch, onDelete,
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-xl)', padding: '1.1rem' }}>
      ...
      <p style={sLbl}>Key</p>
      <input value={keyInput} onChange={(e) => setKeyInput(e.target.value.toLowerCase())}
        onKeyDown={(e) => { if (e.key === 'Enter') onInsert(); }}
        placeholder="e.g. planet" />

      <p style={sLbl}>Value</p>
      ...
      <div style={{ display: 'flex', gap: '0.35rem' }}>
        {[['Slow', 900], ['Normal', 500], ['Fast', 220]].map(([l, v]) => (
